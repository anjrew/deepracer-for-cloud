...
logger.info("Andrew Sanity TEST in training_worker.py" )
...
def training_worker(graph_manager, task_parameters, user_batch_size,
    ...
    try:
        logger.info("Andrew Sanity TEST Creating graph" )
        # initialize graph
        graph_manager.create_graph(task_parameters)

        logger.info("Andrew Sanity TEST Saving Checkpoint" )
        ...
        logger.info(f"Andrew Sanity TEST training_worker.improve_steps.num_steps {graph_manager.improve_steps.num_steps}")
        while steps < graph_manager.improve_steps.num_steps:
            ...
            with utils.Profiler(s3_bucket=PROFILER_S3_BUCKET, s3_prefix=PROFILER_S3_PREFIX,
                ...
                logger.info(f"Andrew Sanity TEST training_worker episodes_in_rollout {episodes_in_rollout}")
                ...
                if graph_manager.should_train():
                    ...
                    graph_manager.phase = core_types.RunPhase.TRAIN
                    screen.log_title(f"Andrew Sanity TEST training_worker train")
...
def main():
    ...
    logger.info("Andrew Sanity TEST 1" )
    ...
    logger.info("Andrew Sanity TEST 2" )
    training_worker(
