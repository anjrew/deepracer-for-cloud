...
class Agent(AgentInterface):
    ...
    """Weather user input will be used to control the model"""
    ...
    def get_controller_state(self):
        """Gets the controller state from an API"""
    ...
    def get_controller_action(self, controller_state):
        ...
        """
        x_axis = controller_state['left_stick_x_axis']
        y_axis = controller_state['right_trigger_axis']
    ...
    def set_user_input_state(self, controller_state):
        """Sets the user input state based on the controller button pressed"""
        user_disabled_input = controller_state['x_button_pressed']
        user_enabled_input = controller_state['a_button_pressed']
        
        if user_enabled_input:
            self.user_input_is_enabled = True
            
        if user_disabled_input:
    ...
    def act(self, action: Union[None, ActionType]=None) -> ActionInfo:
        ...
        if self._phase == RunPhase.TRAIN:
            try:  
                ...
                meta_data = self.ap.env_agent.ctrl._model_metadata_ # type: ignore
        ...
        if action is None:
            if self.phase == RunPhase.HEATUP and not self.ap.algorithm.heatup_using_network_decisions:
                # random action
                action = self.spaces.action.sample_with_info() # type: ignore
            else:
                # informed action
                if self.pre_network_filter is not None:
