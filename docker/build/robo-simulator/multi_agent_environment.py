...
class MultiAgentEnvironment(EnvironmentInterface):
    ...
    def step(self, action: Union[List[ActionType], ActionType]) -> List[EnvResponse]:
        ...
        """
        print("MultiAgentEnvironment.step() action: {}".format(action))
        
        clipped_and_scaled_action = list()
        for agent_action, action_space in zip(force_list(action), force_list(self.action_space)):
            print("MultiAgentEnvironment.step() agent_action: {}".format(agent_action))
            agent_action = action_space.clip_action_to_space(agent_action)
            print("MultiAgentEnvironment.step() agent_action clipped: {}".format(agent_action))
            ...
            print("MultiAgentEnvironment.step() agent_action scaled: {}".format(agent_action))
            clipped_and_scaled_action.append(agent_action)
        action = clipped_and_scaled_action
        print("MultiAgentEnvironment.step() action clipped and scaled: {}".format(action))
        ...
        print("MultiAgentEnvironment.step() self.current_episode_steps_counter: {}".format(self.current_episode_steps_counter))
        # act
    ...
    def get_random_action(self) -> Union[List[ActionType], ActionType]:
        ...
        """
        print("MultiAgentEnvironment.get_random_action() self.action_space: {}".format(self.action_space))
