...
COACH_VER=$(docker inspect awsdeepracercommunity/deepracer-rlcoach:$DR_COACH_IMAGE_TAG 2>/dev/null | jq -r .[].Config.Labels.version)
if [ -z "$COACH_VER" ]; then COACH_VER=$DR_COACH_IMAGE_TAG; fi
...
function dr-test-reward-function {
  ...
  docker run \
    ...
    -v $DR_LOCAL_S3_REWARD_KEY:/data/reward_function.py\
